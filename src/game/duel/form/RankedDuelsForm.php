...
final class RankedDuelsForm extends SimpleForm{
	...
	public function __construct(CollapsePlayer $player){
		...
		parent::__construct(static function(CollapsePlayer $player, ?int $data = null) use ($modes, $queueManager) : void{
			...
			if(isset($modes[$data])){
				$queueManager->joinSoloQueue($player, $modes[$data], true);
			}
		});
		$translator = $player->getProfile()->getTranslator();
		$this->setTitle(CollapseUI::HEADER_FORM_GRID . Font::bold($translator->translate(CollapseTranslationFactory::ranked_duels_form_title())));
		foreach($modes as $mode){
			$this->addButton(
				$translator->translate(CollapseTranslationFactory::duels_form_button_ranked_mode(
					Font::bold($mode->toDisplayName()),
					(string) $queueManager->getSoloRankedInQueue($mode),
					(string) $duelManager->getPlaying(DuelType::Ranked, $mode)
				)),
